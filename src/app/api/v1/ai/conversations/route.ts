...
export async function POST(req: Request) {
  return handleApi(req, "ai.access", async (ctx: ServerContext) => {
    try {
      ...
      logger.error("API create conversation error:", { error: error instanceof Error ? error.message : String(error), stack: error instanceof Error ? error.stack : undefined });
      return NextResponse.json(
...
export async function DELETE(req: Request) {
  return handleApi(req, "ai.access", async (ctx: ServerContext) => {
    try {
      ...
      logger.error("API delete conversation error:", { error: error instanceof Error ? error.message : String(error), stack: error instanceof Error ? error.stack : undefined });
      return NextResponse.json(
        { error: "Internal server error" },
        { status: 500, headers: getCorsHeaders(req) }
      );
