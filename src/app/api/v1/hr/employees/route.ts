...
export async function POST(req: Request) {
  return handleApi(req, "hr.employees.manage", async (ctx: ServerContext) => {
    try {
      ...
      logger.error("API create employee error:", { error: error instanceof Error ? error.message : String(error), stack: error instanceof Error ? error.stack : undefined });
      return NextResponse.json(
...
export async function DELETE(req: Request) {
  return handleApi(req, "hr.employees.manage", async (ctx: ServerContext) => {
    try {
      ...
      logger.error("API delete employee error:", { error: error instanceof Error ? error.message : String(error), stack: error instanceof Error ? error.stack : undefined });
      return NextResponse.json(
        { error: "Internal server error" },
        { status: 500, headers: getCorsHeaders(req) }
      );
