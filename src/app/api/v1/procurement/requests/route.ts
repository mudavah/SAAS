import { NextResponse } from "next/server";
import { db } from "@/db";
import { procurementPurchaseRequests } from "@/db/schema";
import { and, eq, desc } from "drizzle-orm";
import { handleApi, type ServerContext } from "@/lib/session";
import { getCorsHeaders, corsResponse } from "@/lib/api/cors";
import { logger } from "@/lib/logger";

export async function OPTIONS(req: Request) {
  return corsResponse(null, 204, req);
}

export async function GET(req: Request) {
  return handleApi(req, "purchasing.requests.manage", async (ctx: ServerContext) => {
    const rows = await db.query.procurementPurchaseRequests.findMany({
      where: eq(procurementPurchaseRequests.organizationId, ctx.organizationId),
      orderBy: (procurementPurchaseRequests, { desc }) => [
        desc(procurementPurchaseRequests.createdAt),
      ],
      limit: 100,
    });
    return NextResponse.json({ data: rows }, { headers: getCorsHeaders(req) });
  });
}

export async function POST(req: Request) {
  return handleApi(req, "purchasing.requests.manage", async (ctx: ServerContext) => {
    try {
      const body = await req.json();
      if (!body.requestNumber || !body.title || !body.requestedDate) {
        return NextResponse.json(
          { error: "requestNumber, title and requestedDate are required" },
          { status: 400, headers: getCorsHeaders(req) }
        );
      }
      const [item] = await db
        .insert(procurementPurchaseRequests)
        .values({
          organizationId: ctx.organizationId,
          userId: ctx.userId!,
          requestNumber: body.requestNumber,
          title: body.title,
          department: body.department || null,
          requesterId: body.requesterId || null,
          status: body.status || "draft",
          priority: body.priority || "medium",
          notes: body.notes || null,
          requestedDate: new Date(body.requestedDate),
          neededBy: body.neededBy ? new Date(body.neededBy) : null,
          currency: body.currency || "KES",
          totalEstimated: body.totalEstimated || "0",
        })
        .returning();
      return NextResponse.json(
        { data: item },
        { status: 201, headers: getCorsHeaders(req) }
      );
    } catch (error) {
      logger.error("API create purchase request error:", { error: error instanceof Error ? error.message : String(error), stack: error instanceof Error ? error.stack : undefined });
      return NextResponse.json(
        { error: "Internal server error" },
        { status: 500, headers: getCorsHeaders(req) }
      );
    }
  });
}

export async function DELETE(req: Request) {
  return handleApi(req, "purchasing.requests.manage", async (ctx: ServerContext) => {
    try {
      const id = new URL(req.url).searchParams.get("id");
      if (!id) {
        return NextResponse.json(
          { error: "id is required" },
          { status: 400, headers: getCorsHeaders(req) }
        );
      }
      await db
        .delete(procurementPurchaseRequests)
        .where(
          and(
            eq(procurementPurchaseRequests.id, id),
            eq(procurementPurchaseRequests.organizationId, ctx.organizationId)
          )
        );
      return NextResponse.json(
        { data: { id } },
        { headers: getCorsHeaders(req) }
      );
    } catch (error) {
      logger.error("API delete purchase request error:", { error: error instanceof Error ? error.message : String(error), stack: error instanceof Error ? error.stack : undefined });
      return NextResponse.json(
        { error: "Internal server error" },
        { status: 500, headers: getCorsHeaders(req) }
      );
    }
  });
}
