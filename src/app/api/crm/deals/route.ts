...
export async function POST(req: Request) {
  ...
  try {
    ...
    try {
      ...
      logger.error("Timeline emit failed (crm.deal.created):", { error: e instanceof Error ? e.message : String(e), stack: e instanceof Error ? e.stack : undefined });
    }

    return NextResponse.json(deal, { status: 201 });
  } catch (error) {
    logger.error("Create deal error:", { error: error instanceof Error ? error.message : String(error), stack: error instanceof Error ? error.stack : undefined });
