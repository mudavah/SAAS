...
export default function ProcurementAnalyticsPage() {
  ...
  useEffect(() => {
    async function load() {
      setLoading(true);
      try {
        const res = await fetch("/api/analytics/procurement", { cache: "no-store" });
        ...
        toast({
          title: "Error",
          description: err instanceof Error ? err.message : "Failed to load procurement data",
          variant: "destructive",
        });
      } finally {
        setLoading(false);
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Purchase orders, spend, and supplier performance
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{data.orders.toLocaleString("en-KE")}</div>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="pb-2">
        ...
        <Card>
          <CardHeader>
            <CardTitle>Purchase by Month</CardTitle>
          </CardHeader>
          <CardContent>
