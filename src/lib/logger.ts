...
interface LoggerOptions {
  level?: LogLevel;
  /** Force JSON output regardless of NODE_ENV (useful in tests). */
  json?: boolean;
  /** Override the destination sink (defaults to console). */
...
export class Logger {
  ...
  /** Return a child logger with merged context (request/correlation scope). */
...
/** Generate a correlation id for a request (also used by request utils). */
export function newRequestId(): string {
  return uuid();
}

/**
 * Wrap an async handler with timing + error logging. Returns the handler's
 * result. `path`/`method` are attached to the child logger.
 */
